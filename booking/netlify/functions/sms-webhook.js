...
// Parse therapist SMS response - CLICKABLE LINKS ONLY
function parseTherapistResponse(messageBody) {
  console.log('🔍 Parsing message:', messageBody);
  
  // Text responses are disabled - only clickable links should be used
  // Return invalid to trigger help message that explains the new process
  console.log('❌ Text responses disabled - only clickable links supported');
...
async function processBookingResponse(action, bookingId, therapist, therapistPhone) {
  try {
    ...
    // Find booking by full booking ID
...
async function sendHelpSMS(phoneNumber) {
  const helpMessage = `📱 BOOKING RESPONSE HELP

To respond to booking requests, please CLICK the Accept/Decline links provided in your booking request SMS.

Text responses are no longer supported - please use the clickable links.

If you need help, call 1300 302542
- Rejuvenators`;
