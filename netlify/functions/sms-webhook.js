...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (response.isLinkResponse) {
      // For link-based responses, redirect to the therapist-response endpoint
      console.log('🔗 Link-based response detected, redirecting to therapist-response endpoint');
      return {
        statusCode: 302,
        headers: {
          'Location': `/.netlify/functions/therapist-response?booking_id=${response.bookingId}&action=${response.action}&therapist_id=${response.therapistId}`
        },
        body: ''
      };
    } else {
      // For text-based responses, process normally
...
// Parse therapist SMS response (now handles both text and link responses)
function parseTherapistResponse(messageBody) {
  console.log('🔍 Parsing message:', messageBody);
  
  // Check for link-based responses first (new format)
  if (messageBody.includes('therapist-response')) {
    // Extract parameters from the link
    const urlMatch = messageBody.match(/therapist-response\?([^&\s]+)/);
    if (urlMatch) {
  ...
  // Fallback to text-based responses (legacy format)
  const patterns = [
    /^ACCEPT\s+(RB\d{4}\d{4})$/,
    /^DECLINE\s+(RB\d{4}\d{4})$/,
    /^A\s+(RB\d{4}-\d{4})$/,
    /^D\s+(RB\d{4}\d{4})$/
  ];
